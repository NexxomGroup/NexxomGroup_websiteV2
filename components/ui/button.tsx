  ...
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-none text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        ...
        outline:
          "border border-primary text-primary bg-background hover:bg-accent hover:text-primary/70 ",
        secondary:
          "bg-secondary text-secondary-foreground hover:bg-secondary/80",
        ghost: "hover:bg-accent border text-primary-foreground hover:text-secondary-foreground",
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, children, ...props }, ref) => {
    ...
    return (
      <Comp
        // Inclinaison du bouton
        className={cn(
          buttonVariants({ variant, size, className }),
          "inline-flex transform -skew-x-12" // Inclinaison dans l'autre sens
        )}
        ref={ref}
        {...props}
      >
        {/* Redresser le contenu */}
        <span className="inline-flex transform skew-x-12">
          {children}
        </span>
      </Comp>
    )
